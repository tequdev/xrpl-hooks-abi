...
const bufferToReadableData = (
  ...
  switch (state.type) {
    ...
    case 'UInt64':
      return buffer.readBigUInt64LE(0) as bigint
    case 'XFL':
    ...
    case 'Array': {
      ...
      const a = state.array.map((s: Field) => {
        const length = getByteLength(s, Buffer.from(buffer.subarray(from)))
        const buf = Buffer.from(buffer.subarray(from, from + length))
        from += length
...
const toHex = (state: Field, value: null | string | number | bigint | Array<string | number | bigint>): string => {
  switch (state.type) {
    case 'AccountID':
      return Buffer.from(decodeAccountID(value as string))
        .toString('hex')
        .toUpperCase()
    case 'UInt8':
    ...
    case 'VarString':
      if (state.binary === true) {
        if (state.byte_length === undefined) throw new Error('byte_length must be defined for binary VarString')
...
const fieldsTotalLength = (fields: Field[], buffer: Buffer) => {
  return fields.reduce((total, cur) => total + getByteLength(cur, Buffer.from(buffer.subarray(total))), 0)
}
...
const parseBuffer = (buffer: Buffer, definitions: Field[]) => {
  ...
  const check = definitions.every((d) => {
    ...
    const currentBuffer = Buffer.from(buffer.subarray(lastIndex, lastIndex + currentIndex))
  ...
  return methods.reduce(
    (prev, method) => {
      prev[method] = (args: ReadArgs<T>) => {
        ...
        const hookStatePrefix = '0076'
        const accountId = Buffer.from(decodeAccountID(account)).toString('hex').toUpperCase()
        const hookStateKey = parseHex(hook_state_definition.key)
        const index = sha512Half(
          `${hookStatePrefix}${accountId}${hookStateKey}${namespace_id}`,
        )
